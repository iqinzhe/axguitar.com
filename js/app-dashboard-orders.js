...
(function () {
    ...
    const OrdersPage = {
        ...
        async showOrderTable() {
            ...
            try {
                const filters = { status: APP.currentFilter || 'all', search: '' };
                const result = await SUPABASE.getOrders(filters, currentFrom, currentFrom + PAGE_SIZE - 1);
                ...
                // 渲染表格行
                const renderOrdersIntoTable = (orders, append) => {
                ...
                const updateLoadMoreArea = () => {
                    ...
                    if (currentFrom < totalCount) {
                        const remaining = totalCount - currentFrom;
                        const loadMoreHtml = `<tr id="loadMoreRow"><td colspan="${totalCols}" style="text-align:center;padding:14px;"><button onclick="APP.loadMoreOrders()" class="btn-small primary" style="padding:10px 32px;font-size:14px;">⬇️ ${lang === 'id' ? 'Muat Lebih Banyak' : '加载更多'} (${remaining} ${lang === 'id' ? 'tersisa' : '剩余'})</button></td></tr>`;
                        tbody.insertAdjacentHTML('beforeend', loadMoreHtml);
                    } else if (totalCount > PAGE_SIZE) {
                        const doneHtml = `<tr id="loadMoreRow"><td colspan="${totalCols}" style="text-align:center;padding:14px;color:var(--text-muted);">✅ ${lang === 'id' ? `Semua ${totalCount} pesanan telah dimuat` : `已加载全部 ${totalCount} 条订单`}</td></tr>`;
                ...
                document.getElementById("app").innerHTML = `
                    <div class="page-header"><h2>📋 ${t('order_list')}</h2><div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ${t('back')}</button><button onclick="APP.printCurrentPage()" class="btn-print">🖨️ ${t('print')}</button></div></div>
                ...
                window._orderTableState = {
                    currentFrom, totalCount, allOrders, totalCols,
                    pageSize: PAGE_SIZE, filters,
                    storeMap,
                    renderOrdersIntoTable
                };
        ...
        async loadMoreOrders() {
            ...
            try {
                const result = await SUPABASE.getOrders(state.filters, state.currentFrom, state.currentFrom + state.pageSize - 1);
        ...
        async viewOrder(orderId) {
            ...
            try {
                ...
                document.getElementById("app").innerHTML = `
                    <div class="page-header"><h2>📄 ${t('order_details')}</h2><div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ${t('back')}</button><button onclick="APP.printOrder('${Utils.escapeAttr(order.order_id)}')" class="btn-print">🖨️ ${t('print')}</button></div></div>
                    <div class="card">
    ...
    console.log('✅ JF.OrdersPage v2.0 初始化完成');
})();

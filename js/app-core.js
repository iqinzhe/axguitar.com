window.APP = {
    ...
    showCustomers: async function() {
        ...
        try {
            ...
            if (!customers || customers.length === 0) {
                ...
                for (var i = 0; i < customers.length; i++) {
                    var c = customers[i];
                    var hasActiveOrder = customerHasActiveOrder[c.id];
                    rows += `<tr>
                        ...
                        <td>
                            ${hasActiveOrder ? 
                                ...
                                `<button onclick="APP.createOrderForCustomer('${c.id}')" style="padding: 4px 8px; font-size: 12px;" class="success">➕ ${lang === 'id' ? 'Buat Order' : '新建订单'}</button>`
                            }
                        </td>
                    </tr>`;
                }
    ...
    createOrderForCustomer: async function(customerId) {
        try {
            ...
            if (existingOrders && existingOrders.length > 0) {
                alert(Utils.lang === 'id' ? 'Nasabah ini masih memiliki order aktif. Harap selesaikan order terlebih dahulu.' : '该客户还有未结清的订单，请先结清当前订单。');
                return;
            }
            ...
            var html = `
                ...
                <div class="card">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="expenseAmount" placeholder="0" style="width:200px; text-align: right;">
                    </div>
                    <div class="form-group">
            ...
            var printContent = `
                ...
                <head>
                    ...
                    <style>
                        ...
                        .remarks { margin-top: 20px; padding: 15px; background: #fef3c7; border-left: 4px solid #f59e0b; border-radius: 4px; font-size: 12px; color: #92400e; }
                        .remarks p { margin: 5px 0; }
                        @media print {
                ...
                <body>
                    ...
                    <div class="remarks">
                        ...
                        <p>3. Keterlambatan lebih dari 7 hari dikenakan denda 5% per bulan. Lebih dari 30 hari, barang jaminan langsung dijual.</p>
                    </div>
    ...
    saveOrder: async function() {
        ...
        if (!name || !ktp || !phone || !collateral || !amount || amount <= 0) {
            alert(Utils.t('fill_all_fields'));
            return;
        }
        
        try {
            var orderData = { customer: { name, ktp, phone, address }, collateral_name: collateral, loan_amount: amount, notes };
            var newOrder = await Order.create(orderData);
            alert(Utils.t('order_created') + "\nID: " + newOrder.order_id);
            this.goBack();
        } catch (error) {
    ...
    showPayment: async function(orderId) {
        ...
        try {
            ...
            if (principalInput && Utils.bindAmountFormat) {
                Utils.bindAmountFormat(principalInput);
            }
        } catch (error) {

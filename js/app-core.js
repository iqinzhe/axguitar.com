window.APP = {
    ...
    navigateTo: function(page, params) {
        params = params || {};
        this.historyStack.push({ page: this.currentPage, orderId: this.currentOrderId, filter: this.currentFilter, keyword: this.searchKeyword });
    ...
    renderLogin: async function() {
        ...
        document.getElementById("app").innerHTML = `
            <div class="card" style="max-width: 400px; margin: 50px auto;">
                ...
                <p style="margin-top: 15px; font-size: 12px; color: #94a3b8;">ℹ️ ${lang === 'id' ? 'Hubungi administrator untuk akun' : '请联系管理员获取账号'}</p>
            </div>
        `;
    },
    ...
    renderDashboard: async function() {
        ...
        var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; flex-wrap: wrap;"><h1>🏦 JF GADAI ENTERPRISE</h1><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button>${this.historyStack.length > 0 ? '<button onclick="APP.goBack()">↩️ ' + t('back') + '</button>' : ''}</div></div>
            ...
            <div class="card"><h3>${t('current_user')}: ${AUTH.user.name} (${AUTH.user.role})</h3><p>🏪 ${lang === 'id' ? 'Toko' : '门店'}: ${storeName}</p><p>📌 ${lang === 'id' ? 'Admin Fee: 30,000 IDR (dibayar saat kontrak) | Bunga: 10% per bulan (dibayar bulanan)' : '管理费: 30,000 IDR (签合同支付) | 利息: 10%/月 (每月支付)'}</p>${!isAdmin ? '<p style="color: #f59e0b;">🔒 ' + (lang === 'id' ? 'Order yang sudah disimpan tidak dapat diubah' : '已保存的订单不可修改') + '</p>' : ''}</div>`;
    ...
    showOrderTable: async function() {
        ...
        var rows = '';
        if (orders.length === 0) rows = '<tr><td colspan="9" style="text-align: center;">' + t('no_data') + '</td></tr>';
        else {
            for (var i = 0; i < orders.length; i++) {
                ...
                rows += `<tr><td>${Utils.escapeHtml(o.order_id)}</td><td>${Utils.escapeHtml(o.customer_name)}</td><td>${Utils.escapeHtml(o.collateral_name)}</td><td>${Utils.formatCurrency(o.loan_amount)}</td><td>${Utils.formatCurrency(o.admin_fee)}</td><td>${Utils.formatCurrency(o.monthly_interest || 0)}</td><td>${o.interest_paid_months} ${lang === 'id' ? 'bulan' : '个月'}</td><td><span class="status-badge ${statusClass}">${statusMap[o.status] || o.status}</span></td><td><button onclick="APP.navigateTo('viewOrder', {orderId: '${o.order_id}'})">${t('view')}</button>${o.status === 'active' ? '<button onclick="APP.navigateTo(\'payment\', {orderId: \'' + o.order_id + '\'})">💰 ' + t('save') + '</button>' : ''}${canEdit ? '<button onclick="APP.navigateTo(\'editOrder\', {orderId: \'' + o.order_id + '\'})">✏️ ' + t('edit') + '</button>' : ''}${isAdmin ? '<button class="danger" onclick="APP.deleteOrder(\'' + o.order_id + '\')">' + t('delete') + '</button>' : ''}${o.is_locked ? '<span title="' + (lang === 'id' ? 'Terkunci' : '已锁定') + '">🔒</span>' : ''}</td></tr>`;
            }
        }
        var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>📋 ${t('order_list')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
            <div class="toolbar"><input type="text" id="searchInput" placeholder="🔍 ${t('search')}..." style="max-width: 300px;" value="${Utils.escapeHtml(this.searchKeyword)}"><button onclick="APP.searchOrders()">${t('search')}</button><button onclick="APP.resetSearch()">${t('reset')}</button><select id="statusFilter" onchange="APP.filterOrders(this.value)"><option value="all" ${this.currentFilter === 'all' ? 'selected' : ''}>${t('total_orders')}</option><option value="active" ${this.currentFilter === 'active' ? 'selected' : ''}>${t('active')}</option><option value="completed" ${this.currentFilter === 'completed' ? 'selected' : ''}>${t('completed')}</option></select><button onclick="APP.navigateTo('createOrder')">➕ ${t('create_order')}</button></div>
            <div class="table-container"><table><thead><tr><th>ID</th><th>${t('customer_name')}</th><th>${t('collateral_name')}</th><th>${t('loan_amount')}</th><th>${lang === 'id' ? 'Admin Fee' : '管理费'}</th><th>${lang === 'id' ? 'Bunga/Bulan' : '月利息'}</th><th>${lang === 'id' ? 'Bunga Dibayar' : '已付利息'}</th><th>${t('status_active')}</th><th>${t('save')}</th></tr></thead><tbody>${rows}</tbody></table></div>`;
        document.getElementById("app").innerHTML = html;
    },
    ...
    showCreateOrder: function() {
        ...
        var lang = Utils.lang;
        var t = (key) => Utils.t(key);
        var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>📝 ${t('create_order')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
            ...
            <div class="toolbar"><button onclick="APP.saveOrder()">💾 ${t('save')}</button><button onclick="APP.goBack()">↩️ ${t('cancel')}</button></div></div>`;
        document.getElementById("app").innerHTML = html;
    },
    ...
    saveOrder: async function() {
        ...
        try {
            ...
            this.goBack();
        } catch (error) { alert(Utils.lang === 'id' ? 'Gagal menyimpan order: ' + error.message : '保存订单失败：' + error.message); }
    },
    ...
    viewOrder: async function(orderId) {
        ...
        try {
            ...
            } else { paymentHistoryRows = `<tr><td colspan="5" style="text-align: center;">${t('no_data')}</td></tr>`; }
            var remainingPrincipal = order.loan_amount - order.principal_paid;
            var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>📄 ${t('view')} ${t('order_list')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
                ...
                <div class="toolbar"><button onclick="APP.goBack()">↩️ ${t('back')}</button>${order.status === 'active' ? '<button onclick="APP.navigateTo(\'payment\', {orderId: \'' + order.order_id + '\'})">💰 ' + t('save') + '</button>' : ''}${isAdmin && order.is_locked ? '<button onclick="APP.unlockOrder(\'' + order.order_id + '\')">🔓 ' + (lang === 'id' ? 'Buka Kunci' : '解锁') + '</button>' : ''}</div></div>`;
            document.getElementById("app").innerHTML = html;
    ...
    unlockOrder: async function(orderId) {
        if (confirm(Utils.lang === 'id' ? 'Buka kunci order ini untuk edit?' : '解锁此订单以进行编辑？')) {
            try { await Order.unlockOrder(orderId); alert(Utils.lang === 'id' ? 'Order telah dibuka kunci' : '订单已解锁'); await this.viewOrder(orderId); } catch (error) { alert('Error: ' + error.message); }
        }
    },
    ...
    showPayment: async function(orderId) {
        ...
        try {
            ...
            var interestOptions = '';
            for (var i = 1; i <= 12; i++) { var amount = currentMonthlyInterest * i; interestOptions += `<option value="${i}">${i} ${lang === 'id' ? 'bulan' : '个月'} (${Utils.formatCurrency(amount)})</option>`; }
            var adminFeeSection = !order.admin_fee_paid ? `<div style="background:#1e293b;padding:15px;border-radius:8px;margin-bottom:15px;"><h4>📋 ${lang === 'id' ? 'Admin Fee' : '管理费'} - ${Utils.formatCurrency(order.admin_fee)}</h4><button onclick="APP.payAdminFee('${order.order_id}')" class="success">✅ ${lang === 'id' ? 'Catat Pembayaran Admin Fee' : '记录管理费支付'}</button></div>` : `<div style="background:#1e293b;padding:15px;border-radius:8px;margin-bottom:15px;"><h4>📋 ${lang === 'id' ? 'Admin Fee' : '管理费'}</h4><p>✅ ${lang === 'id' ? 'Sudah dibayar' : '已支付'}</p></div>`;
            var principalSection = remainingPrincipal > 0 ? `<div class="form-group"><label>${lang === 'id' ? 'Jumlah Pembayaran Pokok' : '本金支付金额'}:</label><input type="number" id="principalAmount" value="${remainingPrincipal}" style="width:200px;"></div><button onclick="APP.payPrincipal('${order.order_id}')" class="success">✅ ${lang === 'id' ? 'Bayar Pokok' : '支付本金'}</button>` : `<p>✅ ${lang === 'id' ? 'Pokok sudah lunas' : '本金已结清'}</p>`;
            var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>💰 ${lang === 'id' ? 'Pembayaran' : '缴费'}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
                <div class="card"><h3>📋 ${lang === 'id' ? 'Informasi Pesanan' : '订单信息'}</h3><p><strong>${t('customer_name')}:</strong> ${Utils.escapeHtml(order.customer_name)}</p><p><strong>ID Pesanan:</strong> ${Utils.escapeHtml(order.order_id)}</p><p><strong>${t('loan_amount')}:</strong> ${Utils.formatCurrency(order.loan_amount)}</p><p><strong>${lang === 'id' ? 'Sisa Pokok' : '剩余本金'}:</strong> ${Utils.formatCurrency(remainingPrincipal)}</p><p><strong>${lang === 'id' ? 'Bunga Bulanan Saat Ini' : '当前月利息'}:</strong> ${Utils.formatCurrency(currentMonthlyInterest)}</p></div>
                <div class="card"><h3>💰 ${lang === 'id' ? 'Pilih Jenis Pembayaran' : '选择支付类型'}</h3>${adminFeeSection}
                <div style="background:#1e293b;padding:15px;border-radius:8px;margin-bottom:15px;"><h4>💰 ${lang === 'id' ? 'Pembayaran Bunga' : '支付利息'}</h4><div class="form-group"><label>${lang === 'id' ? 'Jumlah Bulan' : '月数'}:</label><select id="interestMonths" style="width:200px;">${interestOptions}</select></div><button onclick="APP.payInterest('${order.order_id}')" class="success">✅ ${lang === 'id' ? 'Catat Pembayaran Bunga' : '记录利息支付'}</button></div>
                <div style="background:#1e293b;padding:15px;border-radius:8px;margin-bottom:15px;"><h4>🏦 ${lang === 'id' ? 'Pembayaran Pokok' : '本金支付'}</h4><p><strong>${lang === 'id' ? 'Sisa Pokok' : '剩余本金'}:</strong> ${Utils.formatCurrency(remainingPrincipal)}</p>${principalSection}</div></div>
                <div class="toolbar"><button onclick="APP.goBack()">↩️ ${t('cancel')}</button></div>`;
            document.getElementById("app").innerHTML = html;
        } catch (error) { alert('Error loading payment page'); this.goBack(); }
    },
    ...
    payAdminFee: async function(orderId) {
        if (confirm(Utils.lang === 'id' ? 'Konfirmasi penerimaan Admin Fee 30,000 IDR?' : '确认已收取管理费 30,000 IDR？')) {
            try { await Order.recordAdminFee(orderId); alert(Utils.lang === 'id' ? 'Admin Fee dicatat!' : '管理费已记录！'); await this.viewOrder(orderId); } catch (error) { alert('Error: ' + error.message); }
        }
    },
    
    payInterest: async function(orderId) {
        var months = parseInt(document.getElementById("interestMonths").value);
        if (confirm((Utils.lang === 'id' ? 'Konfirmasi pembayaran bunga' : '确认支付利息') + '\n' + months + ' ' + (Utils.lang === 'id' ? 'bulan' : '个月'))) {
            try { await Order.recordInterestPayment(orderId, months); alert(Utils.lang === 'id' ? 'Pembayaran bunga dicatat!' : '利息支付已记录！'); await this.viewOrder(orderId); } catch (error) { alert('Error: ' + error.message); }
        }
    },
    ...
    payPrincipal: async function(orderId) {
        ...
        if (confirm((Utils.lang === 'id' ? 'Konfirmasi pembayaran pokok' : '确认支付本金') + ' ' + Utils.formatCurrency(amount) + '?')) {
            try { await Order.recordPrincipalPayment(orderId, amount); alert(Utils.lang === 'id' ? 'Pembayaran pokok dicatat!' : '本金支付已记录！'); await this.viewOrder(orderId); } catch (error) { alert('Error: ' + error.message); }
        }
    },
    ...
    editOrder: async function(orderId) {
        ...
        try {
            ...
            if (!canEdit) { alert(lang === 'id' ? 'Order ini sudah terkunci dan tidak dapat diedit' : '此订单已锁定，无法编辑'); this.goBack(); return; }
            var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>✏️ ${t('edit')} ${t('order_list')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
                <div class="card"><div class="form-group"><label>${t('customer_name')}</label><input id="name" value="${Utils.escapeHtml(order.customer_name)}"></div>
                ...
                <div class="toolbar"><button onclick="APP.updateOrder('${order.order_id}')">💾 ${t('save')}</button><button onclick="APP.goBack()">↩️ ${t('cancel')}</button></div></div>`;
            document.getElementById("app").innerHTML = html;
        } catch (error) { alert('Error loading order for edit'); this.goBack(); }
    },
    
    updateOrder: async function(orderId) {
        var updates = { customer: { name: document.getElementById("name").value, ktp: document.getElementById("ktp").value, phone: document.getElementById("phone").value, address: document.getElementById("address").value }, collateral_name: document.getElementById("collateral").value, notes: document.getElementById("notes").value };
        try { await Order.update(orderId, updates); if (AUTH.isAdmin()) await Order.relockOrder(orderId); alert(Utils.t('order_updated')); this.goBack(); } catch (error) { alert('Error: ' + error.message); }
    },
    
    deleteOrder: async function(orderId) { if (confirm(Utils.t('confirm_delete'))) { try { await Order.delete(orderId); alert(Utils.t('order_deleted')); await this.showOrderTable(); } catch (error) { alert('Error: ' + error.message); } } },
    ...
    showReport: async function() {
        ...
        var lang = Utils.lang;
        var t = (key) => Utils.t(key);
        var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>📊 ${t('financial_report')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
            <div class="stats-grid"><div class="stat-card"><div class="stat-value">${report.total_orders}</div><div>${t('total_orders')}</div></div>
            <div class="stat-card"><div class="stat-value">${Utils.formatCurrency(report.total_loan_amount)}</div><div>${t('total_loan')}</div></div>
            <div class="stat-card"><div class="stat-value">${Utils.formatCurrency(report.total_admin_fees)}</div><div>${lang === 'id' ? 'Admin Fee' : '管理费'}</div></div>
            ...
            <div class="toolbar"><button onclick="Storage.exportOrdersToCSV()">📎 ${lang === 'id' ? 'Ekspor CSV' : '导出CSV'}</button></div>`;
        document.getElementById("app").innerHTML = html;
    },
    ...
    showPaymentHistory: async function() {
        ...
        try {
            ...
            for (var i = 0; i < allPayments.length; i++) { var p = allPayments[i]; if (p.type === 'admin_fee') totalAdminFee += p.amount; else if (p.type === 'interest') totalInterest += p.amount; else if (p.type === 'principal') totalPrincipal += p.amount; }
            var typeMap = { admin_fee: lang === 'id' ? 'Admin Fee' : '管理费', interest: lang === 'id' ? 'Bunga' : '利息', principal: lang === 'id' ? 'Pokok' : '本金' };
            var rows = '';
            if (allPayments.length === 0) rows = '<tr><td colspan="8" style="text-align: center;">' + Utils.t('no_data') + '</td></tr>';
            else { for (var i = 0; i < allPayments.length; i++) { var p = allPayments[i]; rows += `<tr><td>${Utils.escapeHtml(p.orders.order_id)}</td><td>${Utils.escapeHtml(p.orders.customer_name)}</td><td>${Utils.formatDate(p.date)}</td><td>${typeMap[p.type] || p.type}</td><td>${p.months ? p.months + ' ' + (lang === 'id' ? 'bln' : '个月') : '-'}</td><td>${Utils.formatCurrency(p.amount)}</td><td>${Utils.escapeHtml(p.description || '-')}</td><td><button onclick="APP.navigateTo('viewOrder', {orderId: '${p.orders.order_id}'})">${Utils.t('view')}</button></td></tr>`; } }
            var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>💰 ${lang === 'id' ? 'Riwayat Pembayaran' : '付款记录'}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${Utils.t('back')}</button></div></div>
                <div class="stats-grid"><div class="stat-card"><div class="stat-value">${Utils.formatCurrency(totalAdminFee)}</div><div>${lang === 'id' ? 'Total Admin Fee' : '管理费总额'}</div></div>
                ...
                <div class="toolbar"><button onclick="Storage.exportPaymentsToCSV()">📎 ${lang === 'id' ? 'Ekspor CSV' : '导出CSV'}</button></div>`;
            document.getElementById("app").innerHTML = html;
        } catch (error) { alert('Error loading payment history'); }
    },
    ...
    showBackupRestore: function() {
        ...
        var lang = Utils.lang;
        var t = (key) => Utils.t(key);
        var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>💾 ${t('backup_restore')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
            <div class="card"><h3>${lang === 'id' ? 'Cadangan Data' : '备份数据'}</h3><button onclick="Storage.backup()">📥 ${lang === 'id' ? 'Ekspor File Cadangan' : '导出备份文件'}</button>
            <h3 style="margin-top: 30px;">${lang === 'id' ? 'Pemulihan Data' : '恢复数据'}</h3><input type="file" id="restoreFile" accept=".json"><button onclick="APP.restoreData()">📤 ${lang === 'id' ? 'Impor Pemulihan' : '导入恢复'}</button>
    ...
    restoreData: async function() {
        ...
        if (confirm(Utils.lang === 'id' ? '⚠️ Memulihkan data akan menimpa semua data yang ada, lanjutkan?' : '⚠️ 恢复数据将覆盖所有现有数据，确定继续吗？')) {
            ...
            if (success) { alert(Utils.lang === 'id' ? 'Pemulihan data berhasil! Sistem akan dimuat ulang.' : '数据恢复成功！系统将重新加载。'); location.reload(); }
    ...
    showUserManagement: async function() {
        ...
        try {
            ...
            for (var i = 0; i < users.length; i++) {
                ...
                userRows += `<tr><td>${Utils.escapeHtml(u.username)}</td><td>${Utils.escapeHtml(u.name)}</td><td>${u.role === 'admin' ? (lang === 'id' ? 'Administrator' : '管理员') : (lang === 'id' ? 'Manajer Toko' : '店长')}</td><td>${Utils.escapeHtml(storeName)}</td><td>${!isCurrent && u.username !== 'admin' ? '<button class="danger" onclick="APP.deleteUser(\'' + u.id + '\')">' + t('delete') + '</button>' : ''}${!isCurrent ? '<button onclick="APP.editUser(\'' + u.id + '\')">✏️ ' + t('edit') + '</button>' : ''}${isCurrent ? '<span style="color: #10b981;">✅ ' + (lang === 'id' ? 'Login Saat Ini' : '当前登录') + '</span>' : ''}</td></tr>`;
            }
            var storeOptions = '<option value="">' + (lang === 'id' ? 'Pilih Toko' : '选择门店') + '</option>';
            for (var i = 0; i < stores.length; i++) storeOptions += `<option value="${stores[i].id}">${Utils.escapeHtml(stores[i].name)}</option>`;
            var html = `<div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;"><h2>👥 ${t('user_management')}</h2><div><button onclick="APP.toggleLanguage()">🌐 ${lang === 'id' ? '中文' : 'Bahasa Indonesia'}</button><button onclick="APP.goBack()">↩️ ${t('back')}</button></div></div>
                ...
                <div class="form-group"><label>${lang === 'id' ? 'Peran' : '角色'} *</label><select id="newRole" style="width:250px;"><option value="store_manager">${lang === 'id' ? 'Manajer Toko' : '店长'}</option><option value="admin">${lang === 'id' ? 'Administrator' : '管理员'}</option></select></div>
                ...
                <div class="card"><h3>${lang === 'id' ? 'Daftar Pengguna' : '用户列表'}</h3><p style="color:#94a3b8;font-size:12px;margin-bottom:10px;">💡 ${lang === 'id' ? 'Total pengguna: ' + users.length : '用户总数: ' + users.length}</p>
                <div class="table-container"><table style="width:100%;"><thead><tr><th>${t('username')}</th><th>${lang === 'id' ? 'Nama' : '姓名'}</th><th>${lang === 'id' ? 'Peran' : '角色'}</th><th>${lang === 'id' ? 'Toko' : '门店'}</th><th>${t('save')}</th></tr></thead><tbody>${userRows}</tbody></table></div></div>`;
            document.getElementById("app").innerHTML = html;
        } catch (error) { alert('Error loading user management'); }
    },
    ...
    addUser: async function() {
        ...
        try { await AUTH.addUser(username, password, name, role, storeId || null); alert((Utils.lang === 'id' ? 'Pengguna "' : '用户 "') + username + '" ' + (Utils.lang === 'id' ? 'berhasil ditambahkan!' : '添加成功！')); document.getElementById("newUsername").value = ''; document.getElementById("newPassword").value = ''; document.getElementById("newName").value = ''; await this.showUserManagement(); } catch (error) { alert('Error: ' + error.message); }
    },
    
    deleteUser: async function(userId) { if (confirm(Utils.lang === 'id' ? 'Hapus pengguna ini?' : '删除此用户？')) { try { await AUTH.deleteUser(userId); alert(Utils.lang === 'id' ? 'Pengguna telah dihapus' : '用户已删除'); await this.showUserManagement(); } catch (error) { alert('Error: ' + error.message); } } },
    
    editUser: async function(userId) {
        var newRole = prompt(Utils.lang === 'id' ? 'Masukkan peran baru (admin/store_manager):' : '输入新角色 (admin/store_manager):');
        if (newRole && (newRole === 'admin' || newRole === 'store_manager')) { try { await AUTH.updateUser(userId, { role: newRole }); alert(Utils.lang === 'id' ? 'Peran diperbarui' : '角色已更新'); await this.showUserManagement(); } catch (error) { alert('Error: ' + error.message); } }
    },
    ...
    addStore: async function() {
        ...
        try { await StoreManager.createStore(code, name, address, phone); alert(Utils.lang === 'id' ? 'Toko berhasil ditambahkan' : '门店添加成功'); await StoreManager.renderStoreManagement(); } catch (error) { alert('Error: ' + error.message); }
    },
    
    editStore: async function(storeId) {
        var newName = prompt(Utils.lang === 'id' ? 'Masukkan nama toko baru:' : '输入新门店名称:');
        if (newName) { try { await StoreManager.updateStore(storeId, { name: newName }); alert(Utils.lang === 'id' ? 'Nama toko diperbarui' : '门店名称已更新'); await StoreManager.renderStoreManagement(); } catch (error) { alert('Error: ' + error.message); } }
    },
    
    deleteStore: async function(storeId) {
        if (confirm(Utils.lang === 'id' ? 'Hapus toko ini? Semua pengguna yang terkait akan kehilangan toko.' : '删除此门店？所有关联用户将失去门店归属。')) {
            try { await StoreManager.deleteStore(storeId); alert(Utils.lang === 'id' ? 'Toko dihapus' : '门店已删除'); await StoreManager.renderStoreManagement(); } catch (error) { alert('Error: ' + error.message); }
        }
    }
};

...
const StoreManager = {
    ...
    suspendStore: async function(storeId) {
        ...
        try {
            ...
            const { error } = await client.from('stores').update({ is_active: false, updated_at: Utils.getLocalDateTime() }).eq('id', storeId);
    ...
    resumeStore: async function(storeId) {
        ...
        try {
            const client = SUPABASE.getClient();
            const { error } = await client.from('stores').update({ is_active: true, updated_at: Utils.getLocalDateTime() }).eq('id', storeId);
            if (error) throw error;
            var store = StoreManager.stores.find(function(s) { return s.id === storeId; });
    ...
    editStore: async function(storeId) {
        ...
        try {
            ...
            var statusText = isActive ? (lang === 'id' ? 'Aktif' : '营业中') : (lang === 'id' ? 'Ditutup' : '已暂停');
            ...
            modal.innerHTML = '' +
                '<div class="modal-content" style="max-width:500px;">' +
                    ...
                    '<div style="margin-bottom:16px;"><span class="status-badge ' + statusBadgeClass + '">' + statusText + '</span></div>' +
                    '<div class="form-group"><label>' + (lang === 'id' ? 'Kode Toko' : '门店编码') + '</label><input value="' + Utils.escapeHtml(store.code) + '" readonly><div class="form-hint">⚠️ ' + (lang === 'id' ? 'Kode tidak dapat diubah' : '编码不可修改') + '</div></div>' +
                    '<div class="form-group"><label>' + (lang === 'id' ? 'Nama Toko' : '门店名称') + ' *</label><input id="editStoreName" value="' + Utils.escapeHtml(store.name) + '"></div>' +
                    '<div class="form-group"><label>' + (lang === 'id' ? 'Alamat' : '地址') + '</label><input id="editStoreAddress" value="' + Utils.escapeHtml(store.address || '') + '"></div>' +
                    '<div class="form-group"><label>' + (lang === 'id' ? 'Telepon' : '电话') + '</label><input id="editStorePhone" value="' + Utils.escapeHtml(store.phone || '') + '"></div>' +
                    '<div class="form-group"><label>📱 ' + (lang === 'id' ? 'Nomor WhatsApp' : 'WhatsApp 号码') + '</label><input id="editStoreWA" value="' + Utils.escapeHtml(store.wa_number || '') + '" placeholder="628xxxxxxxxxx"><div class="form-hint">' + (lang === 'id' ? 'Contoh: 6281234567890 (tanpa +)' : '示例: 6281234567890 (不带+)') + '</div></div>' +
                    '<div class="modal-actions"><button onclick="StoreManager._saveEditStore(\'' + storeId + '\')" class="success">💾 ' + t('save') + '</button><button onclick="document.getElementById(\'editStoreModal\').remove()" class="btn-back">✖ ' + t('cancel') + '</button></div>' +
                '</div>';
    ...
    _saveEditStore: async function(storeId) {
        ...
        try {
            var updates = { name: name, address: address || null, phone: phone || null, updated_at: Utils.getLocalDateTime() };
    ...
    updateStoreWANumber: async function(storeId, waNumber) {
        ...
        try {
            const client = SUPABASE.getClient();
            const { error } = await client.from('stores').update({ wa_number: waNumber || null, updated_at: Utils.getLocalDateTime() }).eq('id', storeId);
            if (error) throw error;
            const idx = StoreManager.stores.findIndex(s => s.id === storeId);
    ...
    _getAllStoreCashFlowBalances: async function() {
        try {
            const client = SUPABASE.getClient();
            const { data: allFlows, error } = await client.from('cash_flow_records').select('store_id, direction, amount, source_target').eq('is_voided', false);
            if (error) return {};
            var balances = {};
    ...
    renderStoreManagement: async function() {
        ...
        document.getElementById("app").innerHTML = '' +
            '<div class="page-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">' +
                ...
                '<div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ' + t('back') + '</button><button onclick="APP.printCurrentPage()" class="btn-print">🖨️ ' + (lang === 'id' ? 'Cetak' : '打印') + '</button></div>' +
            '</div>' +
            '<div class="card" style="text-align:center; padding:40px;"><div class="loader"></div><p>' + (lang === 'id' ? 'Memuat data toko...' : '加载门店数据中...') + '</p></div>';
        ...
        try {
            ...
            const [allOrdersResult, allExpensesResult, allPaymentsResult] = await Promise.all([
                client.from('orders').select('id, store_id, status, loan_amount, admin_fee_paid, admin_fee, interest_paid_total, principal_paid, service_fee_paid').eq('is_practice', false),
            ...
            const storeStats = {};
            for (var i = 0; i < StoreManager.stores.length; i++) {
                const s = StoreManager.stores[i];
                if (!s.is_practice) {
                    storeStats[s.id] = { orders: [], expenses: [], payments: [] };
                }
            }
            ...
            var grandTotal = { orders: 0, active: 0, loan: 0, adminFee: 0, serviceFee: 0, interest: 0, principal: 0, expenses: 0, income: 0, cashBalance: 0, bankBalance: 0 };
            var storeStatsRows = '';
            
            // 只显示非练习门店的统计
            const displayStores = StoreManager.stores.filter(function(s) { return !s.is_practice; });
            
            for (var i = 0; i < displayStores.length; i++) {
                var store = displayStores[i];
                ...
                var activeCount = 0;
                var totalLoan = 0, totalAdminFee = 0, totalServiceFee = 0, totalInterest = 0, totalPrincipal = 0;
                ...
                var totalExpenses = 0;
                for (var j = 0; j < expenses.length; j++) { totalExpenses += (expenses[j].amount || 0); }
                ...
                grandTotal.orders += ordsCount; grandTotal.active += activeCount; grandTotal.loan += totalLoan;
                grandTotal.adminFee += totalAdminFee; grandTotal.serviceFee += totalServiceFee;
                grandTotal.interest += totalInterest; grandTotal.principal += totalPrincipal;
                grandTotal.expenses += totalExpenses; grandTotal.income += totalIncome;
                grandTotal.cashBalance += cashBalance; grandTotal.bankBalance += bankBalance;
                var storeStatusBadge = store.is_active === false ? ' <span class="status-badge liquidated">' + (lang === 'id' ? 'DITUTUP' : '已暂停') + '</span>' : '';
                storeStatsRows += '<tr>' +
                    ...
                    '<td class="text-center">' + ordsCount + '</td><td class="text-center">' + activeCount + '</td>' +
            ...
            var summaryRow = '<tr style="background:#f1f5f9; font-weight:bold;"><td class="store-name-cell"><strong>📊 ' + (lang === 'id' ? 'TOTAL SEMUA TOKO' : '全部门店合计') + '</strong></td>' +
            ...
            for (var i = 0; i < StoreManager.stores.length; i++) {
                ...
                var statusBadge = isPractice 
                    ? '<span class="status-badge" style="background:#f59e0b;color:#fff;">🎓 ' + (lang === 'id' ? 'Latihan' : '练习') + '</span>'
                    : (isActive 
                ...
                storeRows += '<tr>' +
                    ...
                    '<td class="store-name">' + Utils.escapeHtml(store.name) + (isPractice ? ' 🎓' : '') + '</td>' +
                ...
                var actionButtons = '<button onclick="StoreManager.editStore(\'' + store.id + '\')" class="btn-small">✏️ ' + t('edit') + '</button>';
                if (isActive) {
                ...
                storeRows += '<tr class="action-row"><td class="action-label">' + t('action') + '</td><td colspan="5"><div class="action-buttons">' + actionButtons + '</div></td></tr>';
            }
            ...
            document.getElementById("app").innerHTML = '' +
                '<div class="page-header" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">' +
                    ...
                    '<div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ' + t('back') + '</button><button onclick="APP.printCurrentPage()" class="btn-print">🖨️ ' + (lang === 'id' ? 'Cetak' : '打印') + '</button></div>' +
                '</div>' +
                '<div class="card cashflow-card"><h3>💰 ' + (lang === 'id' ? 'RINGKASAN ARUS KAS' : '现金流汇总') + '</h3>' +
                    '<div class="cashflow-stats">' +
                        '<div class="cashflow-item-card"><div class="label">🏦 ' + (lang === 'id' ? 'Brankas (Tunai)' : '保险柜 (现金)') + '</div><div class="value ' + (grandTotal.cashBalance < 0 ? 'negative' : '') + '">' + Utils.formatCurrency(grandTotal.cashBalance) + '</div></div>' +
                        '<div class="cashflow-item-card"><div class="label">🏧 ' + (lang === 'id' ? 'Bank BNI' : '银行 BNI') + '</div><div class="value ' + (grandTotal.bankBalance < 0 ? 'negative' : '') + '">' + Utils.formatCurrency(grandTotal.bankBalance) + '</div></div>' +
                        '<div class="cashflow-item-card"><div class="label">📊 ' + (lang === 'id' ? 'Total Kas' : '总现金') + '</div><div class="value">' + Utils.formatCurrency(grandTotal.cashBalance + grandTotal.bankBalance) + '</div></div>' +
                    '</div>' +
                '</div>' +
                '<div class="card"><h3>📊 ' + (lang === 'id' ? 'Ringkasan Keuangan Toko' : '门店财务汇总') + '</h3>' +
                    '<div class="table-container" style="overflow-x: auto;"><table class="data-table store-stats-table" style="min-width: 800px;"><thead><tr>' +
                        '<th>' + (lang === 'id' ? 'Toko' : '门店') + '</th><th class="text-center">' + t('total_orders') + '</th><th class="text-center">' + t('active') + '</th>' +
                        '<th class="amount">' + t('total_loan') + '</th><th class="amount">' + (lang === 'id' ? 'Admin Fee' : '管理费') + '</th><th class="amount">' + (lang === 'id' ? 'Service Fee' : '服务费') + '</th>' +
                        '<th class="amount">' + (lang === 'id' ? 'Bunga' : '利息') + '</th><th class="amount">' + (lang === 'id' ? 'Pokok' : '本金') + '</th>' +
                        '<th class="amount">' + (lang === 'id' ? 'Pendapatan' : '收入') + '</th><th class="amount">' + (lang === 'id' ? 'Pengeluaran' : '运营支出') + '</th>' +
                        '<th class="amount">🏦 ' + (lang === 'id' ? 'Brankas' : '保险柜') + '</th><th class="amount">🏧 ' + (lang === 'id' ? 'Bank BNI' : '银行BNI') + '</th>' +
                    '</tr></thead><tbody>' + storeStatsRows + summaryRow + '</tbody></table></div></div>' +
                '<div class="card"><h3>' + (lang === 'id' ? 'Daftar Toko' : '门店列表') + '</h3>' +
                    '<div class="table-container"><table class="data-table store-table"><thead><tr>' +
                        '<th class="col-id">' + (lang === 'id' ? 'Kode' : '编码') + '</th><th class="col-name">' + (lang === 'id' ? 'Nama' : '名称') + '</th>' +
                        '<th class="col-address">' + (lang === 'id' ? 'Alamat' : '地址') + '</th><th class="col-phone">' + (lang === 'id' ? 'Telepon' : '电话') + '</th>' +
                        '<th class="col-phone">📱 WA</th><th class="col-status text-center">' + (lang === 'id' ? 'Status' : '状态') + '</th>' +
                    '</tr></thead><tbody>' + storeRows + '</tbody></table></div></div>' +
                '<div class="card"><h3>' + (lang === 'id' ? 'Tambah Toko Baru' : '新增门店') + '</h3>' +
                    '<div class="form-grid">' +
                        '<div class="form-group"><label>' + (lang === 'id' ? 'Nama Toko' : '门店名称') + ' *</label><input id="newStoreName" placeholder="' + (lang === 'id' ? 'Contoh: Bangil, Gempol' : '例如: Bangil, Gempol') + '"></div>' +
                        ...
                        '<div class="form-actions"><button onclick="APP.addStore()" class="success">➕ ' + (lang === 'id' ? 'Tambah Toko' : '添加门店') + '</button></div>' +
                    '</div>' +
                '</div>';
            
        } catch (error) {
            document.getElementById("app").innerHTML = '' +
                '<div class="page-header"><h2>🏪 ' + (lang === 'id' ? 'Manajemen Toko' : '门店管理') + '</h2>' +
                    '<div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ' + t('back') + '</button></div></div>' +
                '<div class="card" style="text-align:center; padding:40px;"><p style="color:var(--danger);">❌ ' + (lang === 'id' ? 'Gagal memuat data: ' : '加载失败：') + error.message + '</p>' +
                    '<button onclick="StoreManager.renderStoreManagement()" class="btn-small" style="margin-top:16px;">🔄 ' + (lang === 'id' ? 'Coba Lagi' : '重试') + '</button></div>';
        }
    }
};

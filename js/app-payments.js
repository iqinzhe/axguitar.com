// app-payments.js - 完整修复版 v5.4
// 保持原有代码格式风格，只修改 showPayment 内部的 HTML
...
const PaymentsModule = {
    ...
    showPayment: async function(orderId) {
        ...
        try {
            ...
            // 管理费和服务费状态（用于紧凑摘要）
            var adminFeeStatus = order.admin_fee_paid ? '✅ Lunas' : '⚠️ Belum';
            var serviceFeeStatusText = (order.service_fee_paid || 0) >= (order.service_fee_amount || 0) ? '✅ Lunas' : '⚠️ Belum lunas';
            ...
            document.getElementById("app").innerHTML = `
                ...
                <div class="card summary-card">
                    ...
                    <div class="summary-row">
                        ...
                        <div class="summary-field"><span class="field-label">📋 ${lang === 'id' ? 'Status Admin Fee' : '管理费状态'}</span><span class="status-text ${order.admin_fee_paid ? 'success' : 'warning'}">${adminFeeStatus}</span></div>
                        <div class="summary-field"><span class="field-label">💰 ${lang === 'id' ? 'Status Service Fee' : '服务费状态'}</span><span class="status-text ${(order.service_fee_paid || 0) >= (order.service_fee_amount || 0) ? 'success' : 'warning'}">${serviceFeeStatusText}</span></div>
                    </div>
                    <div class="summary-note">ℹ️ ${lang === 'id' ? 'Admin Fee dan Service Fee telah dibayar saat pembuatan order.' : '管理费和服务费已在创建订单时缴纳。'}</div>
                </div>
                ...
                <style>
                    ...
                    .summary-row { display: flex; flex-wrap: wrap; gap: 16px; align-items: center; margin-bottom: 6px; }
                    .summary-field { display: flex; align-items: baseline; gap: 6px; font-size: 0.8rem; }
                    .field-label { color: #64748b; font-weight: 500; min-width: 70px; }
                    .field-value { font-weight: 600; color: #1e293b; }
                    .field-value.warning { color: #f59e0b; }
                    .field-value.success { color: #10b981; }
                    .status-text { font-size: 0.75rem; padding: 2px 6px; border-radius: 12px; }

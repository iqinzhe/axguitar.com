            ...
            try {
                ...
                const buildBlacklistTable = () => {
                    ...
                    if (blacklistTotalCount > blacklist.length) {
                        loadMoreHtml = `<tr id="blacklistLoadMoreRow"><td colspan="4" style="text-align:center;padding:10px;"><button onclick="APP.loadMoreBlacklist()" class="btn btn--sm btn--primary">⬇️ ${lang==='id'?'Muat Lebih Banyak':'加载更多'} (${blacklistTotalCount - blacklist.length} ${lang==='id'?'tersisa':'剩余'})</button></td></tr>`;
                    }

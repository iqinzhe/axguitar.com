        ...
        _updateDashboardDetails(details, lang, isAdmin, kpiReport) {
            ...
            if (messagePreview) {
                ...
                if (pendingCount === 0) {
                    ...
                    if (pendingCount > 3) previewHtml += '<div class="preview-more" onclick="JF.MessageCenter.showMessageCenter()">' + (lang === 'id' ? 'dan ' + (pendingCount - 3) + ' pesan lainnya...' : '还有 ' + (pendingCount - 3) + ' 条消息...') + '</div>';
                    previewHtml += '</div>';
    ...
    <div class="kpi-card kpi-card--calendar">${calendarHTML}</div>
        ...
        <div class="dashboard-v2">
            ...
            <div class="dash-sidebar" id="dashSidebar">
                ...
                ${sidebarMenuHtml}
                <div class="sidebar-footer"><div class="lang-toggle"><div class="lang-btn-side${Utils.lang === 'zh' ? ' active-lang' : ''}" onclick="JF.DashboardCore._setLang('zh')">中文</div><div class="lang-btn-side${Utils.lang === 'id' ? ' active-lang' : ''}" onclick="JF.DashboardCore._setLang('id')">Bahasa</div><div class="lang-btn-side lang-btn-refresh" onclick="JF.DashboardCore.invalidateDashboardCache()" title="${lang === 'id' ? 'Perbarui data' : '刷新数据'}">🔄 ${lang === 'id' ? 'Perbarui' : '刷新数据'}</div><div class="lang-btn-side lang-btn-exit" onclick="JF.DashboardCore.logout()">🚪 ${t('save_exit')}</div></div></div>
            </div>
            ...
            <div class="dash-main">
                ...
                if (!document.getElementById('messageCenterStyle')) {
                    ...
                    style.textContent = `.message-center-card{background:#fff;border-radius:12px;border:1px solid #c8d8e8;box-shadow:0 1px 3px rgba(14,116,144,0.06);padding:12px;transition:box-shadow 0.2s ease,transform 0.2s ease}.message-center-card:hover{box-shadow:0 4px 14px rgba(14,116,144,0.1);transform:translateY(-1px)}.message-badge{background:#ef4444;color:#fff;font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;margin-left:8px}.message-preview{margin-top:8px;flex:1}.empty-preview{text-align:center;padding:20px;color:#94a3b8;font-size:13px}.preview-list{display:flex;flex-direction:column;gap:8px}.preview-item{display:flex;align-items:center;gap:8px;padding:8px 10px;background:#f8fafc;border-radius:8px;font-size:12px;border-left:3px solid #cbd5e1}.preview-item.urgent{border-left-color:#ef4444;background:#fef2f2}.preview-order{font-weight:600;color:#0e7490;min-width:80px}.preview-name{flex:1;color:#1e293b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.preview-badge{font-size:10px;padding:2px 6px;border-radius:12px;background:#e2e8f0;white-space:nowrap}.preview-item.urgent .preview-badge{background:#fee2e2;color:#dc2626}.preview-more{text-align:center;font-size:11px;color:#64748b;padding:6px;cursor:pointer}.preview-more:hover{color:#0e7490;text-decoration:underline}.@keyframes dashFadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}.kpi-card,.fund-flow-card,.income-card,.quick-card,.order-status-card,.message-center-card{animation:dashFadeIn 0.4s ease forwards}.kpi-card:nth-child(2){animation-delay:0.05s}.kpi-card:nth-child(3){animation-delay:0.10s}.kpi-card:nth-child(4){animation-delay:0.15s}`;

// app-dashboard-core.js - v1.5 (资金管理增强版)
// 新增：资金占用卡片、资本注入入口、利润再投入入口
...
const DashboardCore = {
    ...
    // ==================== 加载资金占用卡片数据 ====================
    _loadCapitalCardData: async function() {
        const lang = Utils.lang;
        const cardContent = document.getElementById('capitalCardContent');
        if (!cardContent) return;
        
        try {
            const report = await SUPABASE.getCapitalUtilizationReport();
            
            const utilizationColor = report.utilization_rate > 75 ? '#f59e0b' : 
                                      (report.utilization_rate > 50 ? '#2563eb' : '#10b981');
            
            cardContent.innerHTML = `
                <div class="capital-stats" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;">
                    <div class="capital-item" style="text-align:center;padding:8px;background:var(--bg-hover);border-radius:12px;">
                        <div class="capital-label" style="font-size:11px;color:var(--text-muted);">💰 ${lang === 'id' ? 'Total Modal' : '总资本'}</div>
                        <div class="capital-value" style="font-size:18px;font-weight:700;color:#2563eb;">${Utils.formatCurrency(report.total_capital)}</div>
                    </div>
                    <div class="capital-item" style="text-align:center;padding:8px;background:var(--bg-hover);border-radius:12px;">
                        <div class="capital-label" style="font-size:11px;color:var(--text-muted);">📋 ${lang === 'id' ? 'Dalam Gadai' : '在押资金'}</div>
                        <div class="capital-value" style="font-size:18px;font-weight:700;color:#d97706;">${Utils.formatCurrency(report.deployed_capital)}</div>
                        <div class="capital-sub" style="font-size:10px;color:var(--text-muted);">${report.deployed_orders_count} ${lang === 'id' ? 'pesanan' : '笔订单'}</div>
                    </div>
                    <div class="capital-item" style="text-align:center;padding:8px;background:var(--bg-hover);border-radius:12px;">
                        <div class="capital-label" style="font-size:11px;color:var(--text-muted);">💵 ${lang === 'id' ? 'Dapat Digunakan' : '可动用资金'}</div>
                        <div class="capital-value" style="font-size:18px;font-weight:700;color:#10b981;">${Utils.formatCurrency(report.available_capital)}</div>
                    </div>
                    <div class="capital-item" style="text-align:center;padding:8px;background:var(--bg-hover);border-radius:12px;">
                        <div class="capital-label" style="font-size:11px;color:var(--text-muted);">📊 ${lang === 'id' ? 'Okupansi' : '占用率'}</div>
                        <div class="capital-value" style="font-size:18px;font-weight:700;color:${utilizationColor};">${report.utilization_rate.toFixed(1)}%</div>
                        <div class="progress-bar" style="margin-top:6px;height:6px;background:#e2e8f0;border-radius:4px;overflow:hidden;">
                            <div class="progress-fill" style="width:${Math.min(report.utilization_rate, 100)}%;height:100%;background:${utilizationColor};border-radius:4px;"></div>
                        </div>
                    </div>
                </div>
                ${Math.abs(report.balance_variance) > 1000 ? `
                <div class="info-bar warning" style="margin-top:12px;padding:6px 12px;font-size:12px;">
                    <span class="info-bar-icon">⚠️</span>
                    <div class="info-bar-content">
                        ${lang === 'id' 
                            ? `Varians Rp ${Utils.formatCurrency(Math.abs(report.balance_variance))} (Perlu rekonsiliasi)`
                            : `差异 ${Utils.formatCurrency(Math.abs(report.balance_variance))} (需盘点核对)`}
                    </div>
                </div>
                ` : ''}
                <div class="capital-health ${report.health_status}" style="margin-top:12px;padding:8px 12px;border-radius:8px;font-size:12px;background:#f0fdf4;color:#065f46;">
                    ${report.health_message}
                </div>
            `;
        } catch (error) {
            console.error('加载资金占用卡片失败:', error);
            cardContent.innerHTML = `
                <div class="info-bar danger" style="margin:0;">
                    <span class="info-bar-icon">⚠️</span>
                    <div class="info-bar-content">
                        ${lang === 'id' ? 'Gagal memuat data okupansi modal' : '加载资金占用数据失败'}
                    </div>
                    <button onclick="DashboardCore._loadCapitalCardData()" style="background:none;border:none;cursor:pointer;">🔄</button>
                </div>
            `;
        }
    },
        ...
        try {
            ...
            if (isAdmin) {
                ...
                '<div class="toolbar admin-grid no-print">' +
                    ...
                    '<button onclick="APP.showCapitalInjectionModal()" class="btn-small" style="background:#8b5cf6;color:white;">💰 ' + (lang === 'id' ? 'Injeksi Modal' : '资本注入') + '</button>' +
                    '<button onclick="APP.showProfitReinvestPage()" class="btn-small" style="background:#8b5cf6;color:white;">🔄 ' + (lang === 'id' ? 'Reinvestasi Keuntungan' : '利润再投入') + '</button>' +
            ...
            document.getElementById("app").innerHTML = '' +
                ...
                // 资金占用卡片
                '<div class="card capital-card" style="margin-bottom:16px;">' +
                    '<div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; margin-bottom:12px;">' +
                        '<h3 style="margin:0;">💰 ' + (lang === 'id' ? 'Okupansi Modal' : '资金占用') + '</h3>' +
                        '<div>' +
                            '<button onclick="APP.showCapitalInjectionModal()" class="btn-small" style="background:#8b5cf6;color:white;">💰 ' + (lang === 'id' ? 'Injeksi' : '注入') + '</button>' +
                            '<button onclick="APP.showCapitalUtilizationDetail()" class="btn-small" style="margin-left:8px;">📊 ' + (lang === 'id' ? 'Detail' : '详情') + '</button>' +
                        '</div>' +
                    '</div>' +
                    '<div id="capitalCardContent" style="min-height:80px;">' +
            ...
            setTimeout(async function() {
                await DashboardCore._loadCapitalCardData();
            }, 100);

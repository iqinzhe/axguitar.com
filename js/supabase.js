...
const SupabaseAPI = {
    ...
    async createOrder(orderData) {
        ...
        const loanSource = orderData.loan_source || 'bank';
        const loanAmount = orderData.loan_amount;
        ...
        while (retryCount < 3) {
            try {
                ...
                // 记录贷款发放的资金流出（type='loan_disbursement'）
                const outflowRecord = {
                    order_id: data.id,
                    date: nowDate,
                    type: 'loan_disbursement',
                    amount: loanAmount,
                    description: `Pencairan pinjaman dari ${loanSource === 'cash' ? 'Brankas' : (loanSource === 'bank' ? 'Bank BNI' : 'Laba Bersih')}`,
                    recorded_by: profile.id,
                    payment_method: loanSource === 'cash' ? 'cash' : (loanSource === 'bank' ? 'bank' : 'profit'),
                    target_pool: loanSource
                };
                await supabaseClient.from('payment_history').insert(outflowRecord);
                
                console.log(`✅ 订单创建成功: ${orderId} (门店: ${targetStoreId}, 资金来源: ${loanSource})`);
                return data;
    ...
    async getCashFlowSummary() {
        ...
        let paymentsQuery = supabaseClient.from('payment_history').select('type, amount, payment_method, target_pool, order_id');
        if (!isAdmin && storeId && orderIds.length > 0) {
            paymentsQuery = paymentsQuery.in('order_id', orderIds);
        } else if (!isAdmin && storeId && orderIds.length === 0) {
            paymentsQuery = paymentsQuery.eq('order_id', -1);
        }
        const { data: payments } = await paymentsQuery;
        
        let cashIncome = 0, bankIncome = 0, profitIncome = 0;
        let cashExpense = 0, bankExpense = 0, profitExpense = 0;
        
        for (const p of payments || []) {
            const method = p.payment_method;
            const target = p.target_pool;
        ...
        let expensesQuery = supabaseClient.from('expenses').select('amount, payment_method');
        if (!isAdmin && storeId) expensesQuery = expensesQuery.eq('store_id', storeId);
        const { data: expenses } = await expensesQuery;
        
        let cashExpenseOps = 0, bankExpenseOps = 0;
        let totalOpsExpense = 0;
        for (const e of expenses || []) {
            totalOpsExpense += e.amount;
            if (e.payment_method === 'cash') cashExpenseOps += e.amount;
            else if (e.payment_method === 'bank') bankExpenseOps += e.amount;
        }
        
        const cashBalance = cashIncome - (cashExpense + cashExpenseOps);
    ...
    async investToShop(shopId, amount, paymentMethod, description, transactionDate) {
        ...
        if (!amount || amount <= 0) throw new Error(Utils.lang === 'id' ? 'Jumlah tidak valid' : '金额无效');

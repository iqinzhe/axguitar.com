...
const CustomersModule = {
    ...
    addCustomer: async function() {
        ...
        if (!name) { if (addBtn) { addBtn.disabled = false; addBtn.textContent = '💾 ' + (lang === 'id' ? 'Simpan Nasabah' : '保存客户'); } Utils.toast.warning(lang === 'id' ? 'Nama nasabah harus diisi' : '客户姓名必须填写'); return; }
        if (!phone) { if (addBtn) { addBtn.disabled = false; addBtn.textContent = '💾 ' + (lang === 'id' ? 'Simpan Nasabah' : '保存客户'); } Utils.toast.warning(lang === 'id' ? 'Nomor telepon harus diisi' : '手机号必须填写'); return; }
        try {
            const profile = await SUPABASE.getCurrentProfile();
            const storeId = profile?.store_id;
            if (!storeId) { if (addBtn) { addBtn.disabled = false; addBtn.textContent = '💾 ' + (lang === 'id' ? 'Simpan Nasabah' : '保存客户'); } Utils.toast.error(lang === 'id' ? 'User tidak memiliki toko' : '用户没有关联门店'); return; }
            if (ktp || phone) {
                try {
                    ...
                    if (blacklistedCustomer) {
                        ...
                        if (ktp && blacklistedCustomer.ktp_number === ktp) {
                            reason = lang === 'id' ? '❌ Nomor KTP ' + ktp + ' sudah terdaftar di blacklist' : '❌ 身份证号 ' + ktp + ' 已被拉黑';
                        } else if (phone && blacklistedCustomer.phone === phone) {
                            reason = lang === 'id' ? '❌ Nomor telepon ' + phone + ' sudah terdaftar di blacklist' : '❌ 手机号 ' + phone + ' 已被拉黑';
                        }
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    const customerData = {
                        customer_id: customerId, store_id: storeId, name: name,
                        ktp_number: ktp || null, phone: phone, occupation: occupation || null,
                    ...
                    if (error) {
                        if (error.code === '23505') { lastError = error; await new Promise(r => setTimeout(r, 50 * Math.pow(2, attempt))); continue; }
                        throw error;
                    }
                    newCustomer = data;
                    break;
    ...
    _saveEditCustomer: async function(customerId) {
        ...
        try {
            ...
            const { error } = await client.from('customers').update({
                name: name, phone: phone, ktp_number: ktp || null, occupation: occupation || null,
    ...
    createOrderForCustomer: async function(customerId) {
        ...
        var t = function(key) { return Utils.t(key); };
        var profile = null;
        try { profile = await SUPABASE.getCurrentProfile(); } catch(e) { Utils.toast.error(lang === 'id' ? 'Gagal memuat data user' : '加载用户数据失败'); return; }
        var isAdmin = profile?.role === 'admin';
        if (isAdmin) { Utils.toast.warning(lang === 'id' ? 'Operasi ini hanya untuk operator toko' : '此操作仅限门店操作员'); return; }
        ...
        try {
            ...
            try { blacklistCheck = await SUPABASE.checkBlacklist(customer.id); } catch(blErr) { console.warn('黑名单检查失败:', blErr.message); }
            ...
            try {
                const client = SUPABASE.getClient();
                const { data: existingOrders } = await client.from('orders').select('status').eq('customer_id', customerId).eq('status', 'active');
                if (existingOrders && existingOrders.length > 0) {
                    Utils.toast.warning(lang === 'id' ? 'Nasabah ini masih memiliki pesanan aktif.' : '该客户还有未结清的订单。');
                    return;
                }
            } catch(ordErr) { console.warn('活跃订单检查失败:', ordErr.message); }
            ...
            document.getElementById("app").innerHTML = '' +
                ...
                '<div class="card">' +
                    '<div class="form-section"><div class="form-section-title"><span class="section-icon">👤</span> ' + t('customer_info') + '</div>' +
                        '<div class="info-display">' +
                        ...
                        '</div>' +
                    '</div>' +
                    '<div class="form-section"><div class="form-section-title"><span class="section-icon">💎</span> ' + t('collateral_info') + '</div>' +
                        '<div class="order-first-row">' +
                            ...
                            '<div class="form-group"><label>' + (lang === 'id' ? 'Keterangan Barang' : '物品备注') + '</label><input id="collateralNote" placeholder="' + (lang === 'id' ? 'Contoh: emas 24k, kondisi baik, tahun 2020' : '例如: 24k金, 状况良好, 2020年') + '"></div>' +
                            '<div class="form-group"><label>' + (lang === 'id' ? 'Jumlah Gadai' : '当金金额') + ' *</label><input type="text" id="amount" placeholder="0" class="amount-input" oninput="APP.recalculateAllFees()"></div>' +
                            '<div class="form-group"><label>' + (lang === 'id' ? 'Sumber Dana' : '资金来源') + '</label><div class="payment-method-selector compact"><label><input type="radio" name="loanSource" value="cash" checked> 🏦 ' + t('cash') + '</label><label><input type="radio" name="loanSource" value="bank"> 🏧 ' + t('bank') + '</label></div></div>' +
                        '</div>' +
                    '</div>' +
                    '<div class="form-section"><div class="form-section-title"><span class="section-icon">💰</span> ' + (lang === 'id' ? 'Rincian Biaya' : '费用明细') + '</div>' +
                        '<div class="fee-cards-row">' +
                            '<div class="fee-card"><div class="fee-card-label">📋 ' + (lang === 'id' ? 'Admin Fee' : '管理费') + '</div><div class="fee-card-body"><input type="text" id="adminFeeInput" value="0" class="amount-input" oninput="APP.onAdminFeeManualChange()"></div><div class="fee-card-hint">💡 ' + (lang === 'id' ? 'Dihitung otomatis' : '自动计算') + '</div></div>' +
                            '<div class="fee-card"><div class="fee-card-label">✨ ' + (lang === 'id' ? 'Service Fee' : '服务费') + '</div><div class="fee-card-body"><select id="serviceFeePercentSelect" onchange="APP.recalculateServiceFee()" style="min-width:80px;">' + Utils.getServiceFeePercentOptions(2) + '</select><input type="text" id="serviceFeeInput" value="0" class="amount-input" oninput="APP.onServiceFeeManualChange()"></div><div class="fee-card-hint">💡 ' + (lang === 'id' ? 'Dihitung otomatis berdasarkan jumlah gadai' : '根据当金金额自动计算') + '</div></div>' +
                        '</div>' +
                        '<div class="payment-method-row"><span class="payment-method-label">📥 ' + (lang === 'id' ? 'Metode Pemasukan' : '入账方式') + '</span><div class="payment-method-selector compact" style="padding:0;"><label><input type="radio" name="feePaymentMethod" value="cash" checked> 🏦 ' + t('cash') + '</label><label><input type="radio" name="feePaymentMethod" value="bank"> 🏧 ' + t('bank') + '</label></div><div class="payment-method-hint">💡 ' + (lang === 'id' ? 'Admin Fee & Service Fee akan dicatat bersama' : '管理费和服务费将一起入账') + '</div></div>' +
                        '<div class="form-group interest-rate-group"><label>📈 ' + (lang === 'id' ? 'Suku Bunga (Pilih)' : '利率（可选）') + '</label><select id="agreedInterestRateSelect" onchange="APP.recalculateAllFees()">' + Utils.getInterestRateOptions(8) + '</select></div>' +
                    '</div>' +
                    '<div class="form-section"><div class="form-section-title"><span class="section-icon">📅</span> ' + (lang === 'id' ? 'Metode Cicilan' : '还款方式') + '</div>' +
                        '<div class="repayment-cards-row">' +
                            '<div class="repayment-card selected" id="flexibleCard" onclick="document.getElementById(\'flexibleRadio\').checked=true;APP.toggleRepaymentForm(\'flexible\')"><div class="repayment-card-header"><input type="radio" name="repaymentType" id="flexibleRadio" value="flexible" checked onchange="APP.toggleRepaymentForm(this.value)"><span class="repayment-card-title">💰 ' + t('flexible_repayment') + '</span></div><div class="repayment-card-desc">' + (lang === 'id' ? 'Bayar bunga dulu, pokok bisa kapan saja' : '先付利息，本金随时可还') + '</div><div class="repayment-card-note">' + (lang === 'id' ? 'Maksimal 10 bulan' : '最长10个月') + '</div></div>' +
                            '<div class="repayment-card" id="fixedCard" onclick="document.getElementById(\'fixedRadio\').checked=true;APP.toggleRepaymentForm(\'fixed\')"><div class="repayment-card-header"><input type="radio" name="repaymentType" id="fixedRadio" value="fixed" onchange="APP.toggleRepaymentForm(this.value)"><span class="repayment-card-title">📅 ' + t('fixed_repayment') + '</span></div><div class="repayment-card-desc">' + (lang === 'id' ? 'Angsuran tetap per bulan (bunga + pokok)' : '每月固定还款（本金+利息）') + '</div><div class="repayment-card-note">' + (lang === 'id' ? 'Pilihan 1-10 bulan' : '可选1-10个月') + '</div></div>' +
                            '<div id="flexibleMaxMonthsCard" class="repayment-card extension-card"><div class="repayment-card-header"><span class="repayment-card-title">📅 ' + (lang === 'id' ? 'Maksimal Perpanjangan' : '最大展期') + '</span></div><div class="extension-select"><select id="maxExtensionMonths"><option value="6">6 ' + (lang === 'id' ? 'bulan' : '个月') + '</option><option value="10" selected>10 ' + (lang === 'id' ? 'bulan' : '个月') + '</option><option value="12">12 ' + (lang === 'id' ? 'bulan' : '个月') + '</option><option value="24">24 ' + (lang === 'id' ? 'bulan' : '个月') + '</option></select></div><div class="repayment-card-note extension-note">' + (lang === 'id' ? 'Batas maksimal perpanjangan bunga sebelum harus lunasi pokok' : '利息延期上限，超出后须结清本金') + '</div></div>' +
                        '</div>' +
                        '<div id="fixedRepaymentForm" style="display:none;" class="fixed-repayment-form"><div class="form-grid"><div class="form-group"><label>📅 ' + (lang === 'id' ? 'Jangka Waktu' : '还款期限') + '</label><select id="repaymentTermSelect" onchange="APP.recalculateAllFees()">' + Utils.getRepaymentTermOptions(5) + '</select></div><div class="form-group"><label>💰 ' + (lang === 'id' ? 'Angsuran Bulanan' : '每月还款') + '</label><input type="text" id="monthlyPaymentInput" value="0" class="amount-input" oninput="APP.onMonthlyPaymentManualChange()"><div class="form-hint">' + (lang === 'id' ? 'Dibulatkan ke Rp 10.000, bisa disesuaikan' : '取整到Rp 10,000，可手动调整') + '</div></div></div></div>' +
                    '</div>' +
                    '<div class="form-section"><div class="form-group full-width"><label>' + t('notes') + '</label><textarea id="notes" rows="2" placeholder="' + t('notes') + '"></textarea></div><div class="form-actions"><button onclick="APP.saveOrderForCustomer(\'' + Utils.escapeAttr(customerId) + '\')" class="success" id="saveOrderBtn">💾 ' + (lang === 'id' ? 'Simpan' : '保存') + '</button><button onclick="APP.goBack()">↩️ ' + t('cancel') + '</button></div></div>' +
                '</div>';
    ...
    saveOrderForCustomer: async function(customerId) {
        ...
        if (serviceFee === 0 && serviceFeePercent > 0) { serviceFee = Math.round(amount * serviceFeePercent / 100); }
        ...
        try {
            const profile = await SUPABASE.getCurrentProfile();
            const storeId = profile?.store_id;
            if (!storeId) { if (saveBtn) { saveBtn.disabled = false; saveBtn.textContent = '💾 ' + (lang === 'id' ? 'Simpan' : '保存'); } Utils.toast.error(lang === 'id' ? 'User tidak memiliki toko' : '用户没有关联门店'); return; }
            const customer = await SUPABASE.getCustomer(customerId);
            ...
            var orderData = {
                ...
                collateral_name: fullCollateralName, loan_amount: amount, notes: notes,
                ...
                agreed_interest_rate: agreedInterestRate, repayment_type: repaymentType,
                repayment_term: repaymentTerm, monthly_fixed_payment: monthlyFixedPayment,
                max_extension_months: maxExtensionMonths
            };
            var newOrder = await Order.create(orderData);
            if (adminFee > 0) { try { await Order.recordAdminFee(newOrder.order_id, feePaymentMethod, adminFee); } catch (adminFeeError) { console.error("管理费收取失败:", adminFeeError); } }
            if (serviceFee > 0) { try { await Order.recordServiceFee(newOrder.order_id, 1, feePaymentMethod); } catch (serviceFeeError) { console.error("服务费收取失败:", serviceFeeError); } }
            if (amount > 0) {
                try {
                    var disbursementDesc = lang === 'id' ? 'Pencairan gadai dari ' + (loanSource === 'cash' ? 'Brankas' : 'Bank BNI') : '当金发放自 ' + (loanSource === 'cash' ? '保险柜' : '银行BNI');
                    await Order.recordLoanDisbursement(newOrder.order_id, amount, loanSource, disbursementDesc);
                } catch (loanError) { console.error("当金发放记录失败:", loanError); }
            }
            var successMsg = repaymentType === 'fixed'
                ? (lang === 'id' ? '✅ Pesanan berhasil dibuat!\n\nID Pesanan: ' + newOrder.order_id + '\nJenis: Cicilan Tetap\nJangka: ' + repaymentTerm + ' bulan\nAngsuran per bulan: ' + Utils.formatCurrency(monthlyFixedPayment) : '✅ 订单创建成功！\n\n订单号: ' + newOrder.order_id + '\n还款方式: 固定还款\n期限: ' + repaymentTerm + '个月\n每月还款: ' + Utils.formatCurrency(monthlyFixedPayment))
            ...
            var amountInput = document.getElementById("amount"); if (amountInput) amountInput.value = '';
            var adminFeeInput = document.getElementById("adminFeeInput"); if (adminFeeInput) { adminFeeInput.value = '0'; delete adminFeeInput.dataset.manual; }
            var serviceFeePercentSelect = document.getElementById("serviceFeePercentSelect"); if (serviceFeePercentSelect) { serviceFeePercentSelect.value = '2'; delete serviceFeePercentSelect.dataset.manual; }
            var serviceFeeInput = document.getElementById("serviceFeeInput"); if (serviceFeeInput) { serviceFeeInput.value = '0'; delete serviceFeeInput.dataset.manual; }
            var cashRadio = document.querySelector('input[name="feePaymentMethod"][value="cash"]'); if (cashRadio) cashRadio.checked = true;
            var loanCashRadio = document.querySelector('input[name="loanSource"][value="cash"]'); if (loanCashRadio) loanCashRadio.checked = true;
            var interestSelect = document.getElementById("agreedInterestRateSelect"); if (interestSelect) interestSelect.value = '8';
            var flexibleRadio = document.getElementById("flexibleRadio"); if (flexibleRadio) { flexibleRadio.checked = true; APP.toggleRepaymentForm('flexible'); }
            APP.recalculateAllFees();
            var monthlyInput = document.getElementById("monthlyPaymentInput"); if (monthlyInput) { monthlyInput.value = '0'; delete monthlyInput.dataset.manual; }
    ...
    recalculateAllFees: function() {
        ...
        var adminFeeInput = document.getElementById('adminFeeInput');
        if (adminFeeInput && !adminFeeInput.dataset.manual) { adminFeeInput.value = Utils.formatNumberWithCommas(adminFee); }
        ...
        var serviceFeeInput = document.getElementById('serviceFeeInput');
        if (serviceFeeSelect && !serviceFeeSelect.dataset.manual) { serviceFeeSelect.value = serviceFeeData.percent; }
        if (serviceFeeInput && !serviceFeeInput.dataset.manual) { serviceFeeInput.value = Utils.formatNumberWithCommas(serviceFeeData.amount); }
        ...
        if (repaymentType === 'fixed') {
            ...
            if (amount > 0 && months > 0) {
                ...
                if (monthlyInput && !monthlyInput.dataset.manual) { monthlyInput.value = Utils.formatNumberWithCommas(rounded); }
            }
        }
    },

    onAdminFeeManualChange: function() { var input = document.getElementById('adminFeeInput'); if (input) input.dataset.manual = 'true'; },
    recalculateServiceFee: function() {
        var select = document.getElementById('serviceFeePercentSelect'); if (select) select.dataset.manual = 'true';
        var amountStr = document.getElementById('amount')?.value || '0';
        var amount = Utils.parseNumberFromCommas(amountStr) || 0;
        var percent = select ? parseFloat(select.value) : 0;
        var fee = Math.round(amount * percent / 100);
        var input = document.getElementById('serviceFeeInput'); if (input) { input.value = Utils.formatNumberWithCommas(fee); input.dataset.manual = 'true'; }
    },
    onServiceFeeManualChange: function() { var input = document.getElementById('serviceFeeInput'); if (input) input.dataset.manual = 'true'; },
    onMonthlyPaymentManualChange: function() { var input = document.getElementById('monthlyPaymentInput'); if (input) input.dataset.manual = 'true'; },
        ...
        try {
            ...
            if (!orders || orders.length === 0) {
                ...
                for (var i = 0; i < orders.length; i++) {
                    ...
                    rows += '<tr class="action-row"><td class="action-label">' + (lang === 'id' ? 'Aksi' : '操作') + '</td><td colspan="6"><div class="action-buttons">' + actionButtons + '</div></td></tr>';
                }
            }
            document.getElementById("app").innerHTML = '' +
                '<div class="page-header"><h2>📋 ' + (lang === 'id' ? 'Order Nasabah' : '客户订单') + ' - ' + Utils.escapeHtml(customer.name) + '</h2><div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ' + t('back') + '</button></div></div>' +
                ...
                '</div>' +
                '<div class="card"><h3>📋 ' + t('order_list') + '</h3><div class="table-container"><table class="data-table"><thead><tr><th class="col-id">ID</th><th class="col-date">' + (lang === 'id' ? 'Tanggal' : '日期') + '</th><th class="col-amount amount">' + t('loan_amount') + '</th><th class="col-amount amount">' + (lang === 'id' ? 'Pokok Dibayar' : '已还本金') + '</th><th class="col-months text-center">' + (lang === 'id' ? 'Bunga Dibayar' : '已付利息') + '</th><th class="col-status text-center">' + (lang === 'id' ? 'Jenis' : '方式') + '</th><th class="col-status text-center">' + (lang === 'id' ? 'Status' : '状态') + '</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
        } catch (error) {
    ...
    showCustomerPaymentHistory: async function(customerId) {
        ...
        try {
            ...
            document.getElementById("app").innerHTML = '' +
                '<div class="page-header"><h2>💰 ' + (lang === 'id' ? 'Riwayat Pembayaran' : '付款记录') + ' - ' + Utils.escapeHtml(customer.name) + '</h2><div class="header-actions"><button onclick="APP.goBack()" class="btn-back">↩️ ' + t('back') + '</button></div></div>' +
                '<div class="card customer-summary">' +
                    '<p><strong>' + t('customer_name') + ':</strong> ' + Utils.escapeHtml(customer.name) + '</p>' +
                    '<p><strong>' + t('phone') + ':</strong> ' + Utils.escapeHtml(customer.phone) + '</p>' +
                    '<p><strong>' + (lang === 'id' ? 'Pekerjaan' : '职业') + ':</strong> ' + Utils.escapeHtml(customer.occupation || '-') + '</p>' +
                '</div>' +
                '<div class="card"><h3>💰 ' + (lang === 'id' ? 'Riwayat Pembayaran' : '付款记录') + '</h3><div class="table-container"><table class="data-table"><thead><tr><th class="col-date">' + t('date') + '</th><th class="col-id">' + t('order_id') + '</th><th class="col-type">' + t('type') + '</th><th class="col-months text-center">' + (lang === 'id' ? 'Bulan' : '月数') + '</th><th class="col-amount amount">' + t('amount') + '</th><th class="col-method text-center">' + (lang === 'id' ? 'Metode' : '支付方式') + '</th><th class="col-desc">' + t('description') + '</th></tr></thead><tbody>' + rows + '</tbody></table></div></div>';
        } catch (error) {

...
window.APP_CUSTOMERS = {
    // 显示客户列表
    showCustomers: async function() {
        window.APP.currentPage = 'customers';
        ...
        try {
            ...
            if (!customers || customers.length === 0) {
                rows = `<tr><td colspan="9" style="text-align: center; padding:20px;">${t('no_data')}</td></tr>`;
            } else {
                for (var c of customers) {
                    ...
                    rows += `<tr>
                        ...
                        <td style="border:1px solid #cbd5e1;padding:8px;">${c.gender === 'male' ? (lang === 'id' ? 'Laki-laki' : '男') : (c.gender === 'female' ? (lang === 'id' ? 'Perempuan' : '女') : '-')}</td>
                        <td style="border:1px solid #cbd5e1;padding:8px;">${c.birth_date ? Utils.formatDate(c.birth_date) + ' (' + c.age + ')' : '-'}</td>
                        ...
                        <td style="border:1px solid #cbd5e1;padding:8px;white-space:nowrap;">
                            <button onclick="APP_CUSTOMERS.editCustomer('${c.id}')" style="padding:4px 8px;font-size:12px;">✏️ ${lang === 'id' ? 'Ubah' : '修改'}</button>
                            ${hasActive
                                ...
                                : `<button onclick="APP_CUSTOMERS.createOrderForCustomer('${c.id}')" style="padding:4px 8px;font-size:12px;" class="success">➕ ${lang === 'id' ? 'Buat Order' : '新开订单'}</button>`
                            }
                            ${PERMISSION.canDeleteCustomer() ? `<button onclick="APP_CUSTOMERS.deleteCustomer('${c.id}')" style="padding:4px 8px;font-size:12px;" class="danger">🗑️ ${t('delete')}</button>` : ''}
                        </td>
                    </tr>`;
                }
            }
            ...
            var genderOptions = `
                <option value="">${lang === 'id' ? 'Pilih' : '请选择'}</option>
                <option value="male">${lang === 'id' ? 'Laki-laki' : '男'}</option>
                <option value="female">${lang === 'id' ? 'Perempuan' : '女'}</option>
            `;
            ...
            document.getElementById("app").innerHTML = `
                        ...
                        <table style="width:100%;border-collapse:collapse;">
                            <thead>
                                <tr style="background:#f8fafc;">
                                    ...
                                    <th style="border:1px solid #cbd5e1;padding:10px;">${lang === 'id' ? 'Jenis Kelamin' : '性别'}</th>
                                    <th style="border:1px solid #cbd5e1;padding:10px;">${lang === 'id' ? 'Tanggal Lahir / Umur' : '出生日期/年龄'}</th>
                ...
                <div class="card">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Jenis Kelamin' : '性别'} *</label>
                            <select id="customerGender">${genderOptions}</select>
                        </div>
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Tanggal Lahir' : '出生日期'} *</label>
                            <input type="date" id="customerBirthDate" onchange="APP_CUSTOMERS.calculateAge()">
                        </div>
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Umur' : '年龄'}</label>
                            <input type="text" id="customerAge" readonly style="background:#f1f5f9;">
                        </div>
                        <div class="form-group">
                            <label>${t('phone')} *</label>
                        ...
                        <div class="form-group full-width">
                            ...
                            <div class="address-option">
                                <label>
                                    <input type="radio" name="livingAddrOpt" value="same" checked onchange="APP_CUSTOMERS.toggleLivingAddress(this.value)">
                                    ${lang === 'id' ? 'Sama dengan KTP' : '同上KTP'}
                                </label>
                                <label>
                                    <input type="radio" name="livingAddrOpt" value="different" onchange="APP_CUSTOMERS.toggleLivingAddress(this.value)">
                        ...
                        <div class="form-actions">
                            <button onclick="APP_CUSTOMERS.addCustomer()" class="success">💾 ${lang === 'id' ? 'Simpan Nasabah' : '保存客户'}</button>
                        </div>
                    </div>
                </div>
                <div class="toolbar">
    ...
    calculateAge: function() {
        var birthDate = document.getElementById("customerBirthDate").value;
        if (birthDate) {
            var today = new Date();
    ...
    // 计算编辑时的年龄
    calculateEditAge: function(birthDate) {
        if (birthDate) {
            var today = new Date();
            var birth = new Date(birthDate);
            var age = today.getFullYear() - birth.getFullYear();
    ...
    addCustomer: async function() {
        ...
        var gender = document.getElementById("customerGender").value;
        var birthDate = document.getElementById("customerBirthDate").value;
        ...
        if (!gender) { alert(lang === 'id' ? 'Jenis kelamin harus dipilih' : '请选择性别'); return; }
        if (!birthDate) { alert(lang === 'id' ? 'Tanggal lahir harus diisi' : '出生日期必须填写'); return; }
        if (!ktp) { alert(lang === 'id' ? 'Nomor KTP harus diisi' : 'KTP号码必须填写'); return; }
        if (!phone) { alert(lang === 'id' ? 'Nomor telepon harus diisi' : '手机号必须填写'); return; }
        if (!ktpAddress) { alert(lang === 'id' ? 'Alamat KTP harus diisi' : 'KTP地址必须填写'); return; }
    ...
    editCustomer: async function(customerId) {
        ...
        try {
            ...
            var genderOptions = `
                <option value="">${lang === 'id' ? 'Pilih' : '请选择'}</option>
                <option value="male" ${c.gender === 'male' ? 'selected' : ''}>${lang === 'id' ? 'Laki-laki' : '男'}</option>
                <option value="female" ${c.gender === 'female' ? 'selected' : ''}>${lang === 'id' ? 'Perempuan' : '女'}</option>
            `;
            ...
            modal.innerHTML = `
                <div style="background:#ffffff;border-radius:12px;padding:24px;width:100%;max-width:600px;max-height:90vh;overflow-y:auto;">
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Jenis Kelamin' : '性别'} *</label>
                            <select id="ec_gender">${genderOptions}</select>
                        </div>
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Tanggal Lahir' : '出生日期'} *</label>
                            <input type="date" id="ec_birthDate" value="${c.birth_date || ''}" onchange="APP_CUSTOMERS.updateEditAge()">
                        </div>
                        <div class="form-group">
                            <label>${lang === 'id' ? 'Umur' : '年龄'}</label>
                            <input type="text" id="ec_age" value="${c.age || ''}" readonly style="background:#f1f5f9;">
                        </div>
                        <div class="form-group">
                            <label>${t('phone')} *</label>
                        ...
                        <div class="form-group full-width">
                            ...
                            <div class="address-option">
                                <label>
                                    <input type="radio" name="ec_livingOpt" value="same" ${livingSame ? 'checked' : ''} onchange="APP_CUSTOMERS.toggleEditLiving(this.value)">
                                    ${lang === 'id' ? 'Sama dengan KTP' : '同上KTP'}
                                </label>
                                <label>
                                    <input type="radio" name="ec_livingOpt" value="different" ${!livingSame ? 'checked' : ''} onchange="APP_CUSTOMERS.toggleEditLiving(this.value)">
                                    ${lang === 'id' ? 'Berbeda' : '不同'}
                                </label>
                            </div>
                        ...
                        <div class="form-actions">
                            <button onclick="APP_CUSTOMERS.saveEditCustomer('${customerId}')" class="success">💾 ${t('save')}</button>
            ...
            // 计算并显示年龄
            if (c.birth_date) {
                var age = this.calculateEditAge(c.birth_date);
                document.getElementById("ec_age").value = age;
            }
        } catch (e) {
    ...
    updateEditAge: function() {
        var birthDate = document.getElementById("ec_birthDate").value;
        if (birthDate) {
            var today = new Date();
            var birth = new Date(birthDate);
            var age = today.getFullYear() - birth.getFullYear();
            ...
            document.getElementById("ec_age").value = '';
        }
    },

    toggleEditLiving: function(val) {
        var el = document.getElementById('ec_livingAddr');
        if (el) el.style.display = val === 'different' ? 'block' : 'none';
    },

    saveEditCustomer: async function(customerId) {
        var lang = Utils.lang;
        var name = document.getElementById('ec_name').value.trim();
        var gender = document.getElementById('ec_gender').value;
        var birthDate = document.getElementById('ec_birthDate').value;
        var age = document.getElementById('ec_age').value;
        ...
        if (!name) { alert(lang === 'id' ? 'Nama harus diisi' : '姓名必须填写'); return; }
        if (!gender) { alert(lang === 'id' ? 'Jenis kelamin harus dipilih' : '请选择性别'); return; }
        if (!birthDate) { alert(lang === 'id' ? 'Tanggal lahir harus diisi' : '出生日期必须填写'); return; }
        if (!phone) { alert(lang === 'id' ? 'Telepon harus diisi' : '手机号必须填写'); return; }
        if (!ktp) { alert(lang === 'id' ? 'Nomor KTP harus diisi' : 'KTP号码必须填写'); return; }
        if (!ktpAddr) { alert(lang === 'id' ? 'Alamat KTP harus diisi' : 'KTP地址必须填写'); return; }
        ...
        try {
            ...
            if (orders && orders.length > 0) {
                ...
                const { error: orderDeleteError } = await supabaseClient
                    ...
                    .eq('customer_id', customerId);
                if (orderDeleteError) {
                    console.error("删除订单错误:", orderDeleteError);
            ...
            if (customerError) {
                console.error("删除客户错误:", customerError);
                throw customerError;
            }
    ...
    createOrderForCustomer: async function(customerId) {
        try {
            ...
            window.APP.currentPage = 'createOrder';
            window.APP.currentCustomerId = customerId;
            window.APP.saveCurrentPageState();
            var lang = Utils.lang;
            var t = (key) => Utils.t(key);
            ...
            document.getElementById("app").innerHTML = `
                ...
                <div class="card">
                    ...
                    <p><strong>${lang === 'id' ? 'Jenis Kelamin' : '性别'}:</strong> ${customer.gender === 'male' ? (lang === 'id' ? 'Laki-laki' : '男') : (customer.gender === 'female' ? (lang === 'id' ? 'Perempuan' : '女') : '-')}</p>
                    <p><strong>${lang === 'id' ? 'Tanggal Lahir' : '出生日期'}:</strong> ${customer.birth_date ? Utils.formatDate(customer.birth_date) + ' (' + customer.age + ')' : '-'}</p>
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-actions">
                            <button onclick="APP_CUSTOMERS.saveOrderWithCustomer('${customerId}')">💾 ${t('save')}</button>
    ...
    showCustomerOrders: async function(customerId) {
        window.APP.currentPage = 'customerOrders';
        window.APP.currentCustomerId = customerId;
        window.APP.saveCurrentPageState();
        var lang = Utils.lang;
        var t = (key) => Utils.t(key);
    ...
    showCustomerPaymentHistory: async function(customerId) {
        window.APP.currentPage = 'customerPaymentHistory';
        window.APP.currentCustomerId = customerId;
        window.APP.saveCurrentPageState();
        var lang = Utils.lang;
        var t = (key) => Utils.t(key);
...
// 将客户管理方法挂载到 APP 对象
window.APP.showCustomers = APP_CUSTOMERS.showCustomers.bind(APP_CUSTOMERS);
window.APP.toggleLivingAddress = APP_CUSTOMERS.toggleLivingAddress.bind(APP_CUSTOMERS);
window.APP.addCustomer = APP_CUSTOMERS.addCustomer.bind(APP_CUSTOMERS);
window.APP.editCustomer = APP_CUSTOMERS.editCustomer.bind(APP_CUSTOMERS);
window.APP._toggleEditLiving = APP_CUSTOMERS.toggleEditLiving.bind(APP_CUSTOMERS);
window.APP._saveEditCustomer = APP_CUSTOMERS.saveEditCustomer.bind(APP_CUSTOMERS);
window.APP.deleteCustomer = APP_CUSTOMERS.deleteCustomer.bind(APP_CUSTOMERS);
window.APP.createOrderForCustomer = APP_CUSTOMERS.createOrderForCustomer.bind(APP_CUSTOMERS);
window.APP.saveOrderWithCustomer = APP_CUSTOMERS.saveOrderWithCustomer.bind(APP_CUSTOMERS);
window.APP.showCustomerOrders = APP_CUSTOMERS.showCustomerOrders.bind(APP_CUSTOMERS);
window.APP.showCustomerPaymentHistory = APP_CUSTOMERS.showCustomerPaymentHistory.bind(APP_CUSTOMERS);
